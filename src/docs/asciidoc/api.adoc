ifndef::snippets[]
:snippets: ./build/generated-snippets
endif::[]

= Rest Docs API
:doctype: book
:icons: font
:source-highlighter: highlightjs
:toc: left
:toclevels: 4
:sectlinks:

## AUTH

=== silent-refresh (토큰 재발급)

==== REQUEST

include::{snippets}/auth/silent-refresh/http-request.adoc[]

==== RESPONSE

include::{snippets}/auth/silent-refresh/http-response.adoc[]

## User

=== User signup (기본 회원가입)

==== REQUEST

include::{snippets}/user/signup/http-request.adoc[]

==== RESPONSE

include::{snippets}/user/signup/http-response.adoc[]

=== User additional-signup (추가 회원가입)

==== REQUEST

include::{snippets}/user/additional-signup/httpie-request.adoc[]

include::{snippets}/user/additional-signup/request-parameters.adoc[]

include::{snippets}/user/additional-signup/request-parts.adoc[]

==== RESPONSE

include::{snippets}/user/additional-signup/http-response.adoc[]

=== User logout (로그아웃)

==== REQUEST

include::{snippets}/user/logout/http-request.adoc[]

==== RESPONSE

include::{snippets}/user/logout/http-response.adoc[]